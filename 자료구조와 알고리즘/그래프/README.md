...
> 2. 서로 다른 집합에 속한 두 노드가 연결된 경우, 두 노드의 집합 번호 중 작은 집합 번호로 두 노드의 집합 번호를 갱신한다.
...
> 4-1. `출발 노드의 최단 거리 + 간선 가중치 < 도착 노드의 최단 거리`일 경우 도착 노드의 최단 거리를 `출발 노드의 최단 거리 + 간선 가중치`로 갱신한다. 출발 노드의 최단 거리가  `INF`인 경우에는 무시한다.
> 4-2. 만약 `도착 노드`가 큐에 들어가지 않은 상태라면 `도착 노드`의 사이클 길이를 늘린다. 이때 사이클 길이가 전체 노드 수와 같아지면 `음의 사이클`이 있다고 판단하고 탐색을 중단한다.
> 4-3. 그렇지 않다면 `도착 노드`를 큐에 넣는다(배열도 갱신).
